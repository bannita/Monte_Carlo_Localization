...
def get_sensor_distance(x,y,theta,grid_map,max_distance):
    ...
    while distance < max_distance:
        check_x = int(x+distance*math.cos(theta))
        check_y = int(y+distance*math.sin(theta))

        if check_x < 0 or check_x >= grid_map.shape[1]:
            return distance
        if check_y < 0 or check_y >= grid_map.shape[0]:
            return distance
        
        if grid_map[check_y,check_x] == 1:
            return distance
...
while True:
    ...
    if angle_concentration is not None:
        ...
        cv2.putText(
            display,
            "Angle: " + str(round(angle_concentration, 2)),
            (10, 115),
    ...
    if position_error is not None:
        ...
        cv2.putText(
            display,
            "Error: " + str(round(position_error, 2)),
            (10, 85),
            cv2.FONT_HERSHEY_SIMPLEX,
            0.6,
            (0, 255, 255),
    ...
    if position_spread is not None:    
        cv2.putText(
            display,
            "Spread: " + str(round(position_spread, 2)),
            (10, 25),
            cv2.FONT_HERSHEY_SIMPLEX,
            0.6,
            (0, 255, 255),
